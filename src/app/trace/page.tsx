    ...
    const fetchTokens = async () => {
      try {
        ...
        const provider = new ethers.JsonRpcProvider(
          'https://eth-mainnet.g.alchemy.com/v2/pIJsemLjqv--j987X_zM9TbCfG1HW9AK'
        );
        ...
        const response = await fetch(
          `https://eth-mainnet.g.alchemy.com/nft/v2/pIJsemLjqv--j987X_zM9TbCfG1HW9AK/getNFTsForContract?${params.toString()}`,
          {
            method: 'GET',
            headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `API Error: ${errorData.message || 'Failed to fetch tokens'}`
          );
        }
        ...
        if (data.nfts) {
          ...
          for (const nft of data.nfts) {
            ...
            const ownerResponse = await fetch(
              `https://eth-mainnet.g.alchemy.com/nft/v2/pIJsemLjqv--j987X_zM9TbCfG1HW9AK/getOwnersForToken?contractAddress=${nft.contract.address}&tokenId=${nft.id.tokenId}`,
              {
                method: 'GET',
                headers: {
                  Accept: 'application/json',
        ...
        <section className="max-w-6xl mx-auto px-4 py-8">
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-black/80 border border-[#00ff00]/20 p-4 text-center">
              <div className="text-[#00ff00] font-['Press_Start_2P'] text-sm mb-2">
                ARTWORKS CREATED
              </div>
              <div className="text-white/90 font-['VT323'] text-2xl">
                {stats.totalTokens}
              </div>
            </div>
            <div className="bg-black/80 border border-[#00ff00]/20 p-4 text-center">
            ...
            {isLoading ? (
              ...
              <div className="space-y-4">
                ...
                {owners.map((owner) => (
                  <div
                    key={owner.address}
                    className="grid grid-cols-2 gap-4 text-white/90 font-['VT323'] text-sm border-b border-[#00ff00]/20 pb-2"
                  >
